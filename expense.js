document.addEventListener('DOMContentLoaded', function () {
    ...
    function createListItem(expense) {
        ...
        li.querySelector('.edit-btn').addEventListener('click', function () {
            ...
            // axios.get(`http://localhost:3000/expense/getexpense/${editId}`, {
            //     headers: { Authorization: `Bearer ${token}` }
            // })
            // .then(response => {
    ...
    function renderExpenses(page) {
        axios.get(`http://localhost:3000/expense/getexpenses?page=${page}&limit=${limit}`, {
            headers: { Authorization: `Bearer ${token}` }
        })
        .then(response => {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (editId !== null) {
            axios.put(`http://localhost:3000/expense/editexpense/${editId}`, newExpense, {
                headers: { Authorization: `Bearer ${token}` }
            })
            .then(() => {
                editId = null;
            ...
            axios.post('http://localhost:3000/expense/addexpense', newExpense, {
                headers: { Authorization: `Bearer ${token}` }
            })
            .then(() => {
                form.reset();
